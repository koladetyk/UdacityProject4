...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('can register a flight', async () => {
    ...
    let result = await config.flightSuretyData.isFlightRegistered(airline, flight, timestamp);
  ...
  it('can submit oracle response', async () => {
    ...
    let indexes = await config.flightSuretyApp.getMyIndexes({from: someOracle});
    let airline = config.firstAirline;
    let flight = 'ND1309';
    let timestamp = Math.floor(Date.now() / 1000);
    ...
    for (let idx = 0; idx < indexes.length; idx++) {
      await config.flightSuretyApp.submitOracleResponse(indexes[idx], airline, flight, timestamp, statusCode, {from: someOracle});
    }
  ...
  it('processes insurance payouts correctly', async () => {
    ...
    await config.flightSuretyApp.buyInsurance(config.firstAirline, flight, timestamp, {from: passenger, value: insuranceAmount});
